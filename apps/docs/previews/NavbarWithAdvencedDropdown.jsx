...
import { Navbar, NavbarContent, NavbarBrand, NavbarNav, NavbarItem, NavbarActions, DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem } from "@timui/react";
import { Button } from "@timui/react";

export default function NavbarWithAdvencedDropdown() {
  const dropdownNavs = [
    {
        label: "Products", navs: [
            {
    ...
    return (
        <div style={{height: "480px"}}>
            <Navbar className={`relative z-20 bg-white w-full md:static md:text-sm md:border-none ${state ? "shadow-lg rounded-b-xl md:shadow-none" : ""}`}>
                <NavbarContent className="items-center gap-x-14 px-4 max-w-screen-xl mx-auto md:flex md:px-8">
                    ...
                    <NavbarNav className={`nav-menu flex-1 pb-3 mt-8 md:block md:pb-0 md:mt-0 ${state ? 'block' : 'hidden'}`}>
                        <ul className="items-center space-y-6 md:flex md:space-x-6 md:space-y-0">
                                        ...
                                        <NavbarItem key={idx}>
                                            {
                                                item.isDrapdown ? (
                                                    <DropdownMenu>
                                                        <DropdownMenuTrigger className="w-full flex items-center justify-between gap-1 text-gray-700 hover:text-indigo-600">
                                                            {item.title}
                                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
                                                                <path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clipRule="evenodd" />
                                                            </svg>
                                                        </DropdownMenuTrigger>
                                                        <DropdownMenuContent className="mt-6 inset-x-0 top-20 w-full md:absolute md:border-y md:shadow-md md:mt-0">
                                                                            ...
                                                                            {dropdownItem.navs.map((navItem, idx) => (
                                                                                <li key={idx} className="group">
                                                                                    <DropdownMenuItem asChild className="outline-none">
                                                                                        <a href={navItem.path} className='flex gap-3 items-center p-2'>
                            ...
                            <NavbarActions className='flex-1 items-center justify-end gap-x-6 space-y-3 md:flex md:space-y-0'>
                                <li>
