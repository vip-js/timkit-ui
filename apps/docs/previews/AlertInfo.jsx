import { Alert } from "@timui/react";

export default function AlertInfo() {
    return (
        <Alert variant="default" className="max-w-2xl mx-auto mt-12 bg-blue-50 text-blue-500">
            <Alert.Title>Info</Alert.Title>
            <Alert.Description>
                <div className="mt-1">
                    New sales from the last subscribers - 20K USD in revenue.
                </div>
                <div className="mt-2">
                    <a href="javascript:void(0)" className="flex items-center text-sm font-medium underline">
                        Details
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 ml-1" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z" clipRule="evenodd" />
                        </svg>
                    </a>
                </div>
            </Alert.Description>
        </Alert>
    )
}
        