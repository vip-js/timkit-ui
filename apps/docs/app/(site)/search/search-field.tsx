      ...
      <div className="relative">
        <MultipleSelector
          ...
          options={getFilteredOptions().filter(
            (option) => !inputValue || option.value.toLowerCase().includes(inputValue.toLowerCase())
          )}
          value={selectedOptions}
          hidePlaceholderWhenSelected
          emptyIndicator={<p className="text-center text-sm">No tags found</p>}
          onChange={handleMultipleSelectorChange}
          className="border-border/60 w-full rounded-2xl bg-card/80 ps-[52px] shadow-[0_10px_24px_-18px_rgba(15,23,42,0.2)]"
        />
        <div
