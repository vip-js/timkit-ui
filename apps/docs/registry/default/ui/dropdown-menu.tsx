...
type DropdownMenuApi = ReturnType<typeof menuConnect>

const DropdownMenuContext = React.createContext<DropdownMenuApi | null>(null)
...
function useDropdownMenu(props: DropdownMenuProps) {
  ...
  const service = useMachine(menuMachine, {
    ...
    positioning: menuProps.positioning as never,
...
const useDropdownMenuContext = (): DropdownMenuApi => {
...
const DropdownMenu = (props: DropdownMenuProps) => {
  ...
  return (
    <DropdownMenuSubRootContext.Provider value={{ openSubId, setOpenSubId }}>
      <DropdownMenuContext.Provider value={api}>{children}</DropdownMenuContext.Provider>
...
type DropdownPlacement = `${'top' | 'bottom' | 'left' | 'right'}${'' | '-start' | '-end'}`
...
const DropdownMenuContent = React.forwardRef<HTMLDivElement, DropdownMenuContentProps>(
  ({ className, side = 'bottom', align = 'center', sideOffset = 4, ...props }, ref) => {
    const api = useDropdownMenuContext()
    const placement = (align === 'center' ? side : `${side}-${align}`) as DropdownPlacement
...
const DropdownMenuItem = React.forwardRef<HTMLElement, DropdownMenuItemProps>(
  (
    ...
    const { className: mergedClassName, ...restProps } =
      mergedProps as React.HTMLAttributes<HTMLElement>
...
const DropdownMenuSub = ({ ...props }: DropdownMenuSubProps) => {
  ...
  return (
    <DropdownMenuSubContext.Provider
      value={{ id, open, setOpen, triggerElement, setTriggerElement }}
    >
...
const DropdownMenuRadioGroup = ({
  ...
  return (
    <DropdownMenuRadioGroupContext.Provider value={groupContextValue}>
