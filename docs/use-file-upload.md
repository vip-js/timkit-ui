...
### Prerequisites
- React 环境（与项目 React 版本一致，建议 19.x）。
- 示例依赖的 UI 组件/图标（`Button`, `UploadIcon`, `AlertCircleIcon` 等）需在你的项目中可用（可替换为自有组件）。
- 推荐使用 `pnpm tokens:build` 产出的样式/变量，保持一致的视觉。
...
### Boundary & Error Handling Checklist
- 限制：通过 `maxSize`/`maxFiles` 兜底，并在 UI 提示。
- 并发：在 `onFilesAdded` 中对上传并发设上限（如用 `p-limit`）。
- 重试：封装上传 Promise，支持重试/指数退避。
- 取消：为每个上传保存 AbortController/xhr 引用，提供取消按钮。
- 状态同步：后端返回的 url/id 需写回文件列表，避免 UI 与服务器状态不一致。
...
For more examples and live demos of the `useFileUpload` hook in action, visit the [Timkit UI File Uploader page](https://originui.com/file-uploader).
