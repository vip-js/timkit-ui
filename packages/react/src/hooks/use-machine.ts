...
export function useMachine(machine: object, options: { context?: MachineContext } = {}) {
    ...
    const send = useCallback((event: MachineEvent) => {
        ...
        const transition = machineRef.states?.[currentStateKey]?.on?.[evt.type] || machineRef.on?.[evt.type]
        ...
        if (transition) {
            ...
            actionList.forEach((actionName: string) => {
                const actionFn = machineRef.options?.actions?.[actionName]
