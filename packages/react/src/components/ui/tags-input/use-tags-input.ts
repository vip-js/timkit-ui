...
export function useTagsInput({ onValueChange, onValueChangeDetails, ...props }: UseTagsInputProps): TagsInputApi {
    ...
    return React.useMemo(() => {
        return Array.isArray(service)
            ? (tagsInputConnect as never as (state: never, send: never, props: typeof normalizeProps) => TagsInputApi)(
                service[0] as never,
                service[1] as never,
                normalizeProps
            )
