...
import { sliderConnect, sliderMachine, createTimEvent } from '@timui/core'
...
export function useSlider(props: UseSliderProps) {
    ...
    const service = useMachine(sliderMachine, {
        ...
        onValueChange(details) {
            const event = createTimEvent('change', sliderId, { value: details.value }) as never as Parameters<NonNullable<CoreSliderProps['onValueChange']>>[0]
            props.onValueChange?.(event)
        },
        onValueChangeEnd(details) {
            const event = createTimEvent('change-end', sliderId, { value: details.value }) as never as Parameters<NonNullable<CoreSliderProps['onValueChangeEnd']>>[0]
