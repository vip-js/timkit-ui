...
- **Native Implementation**: 各框架包 (`packages/react` 等) 编写原生代码，保证最佳性能和 SSR 支持。
...
- `npx timkit add <name> --framework <react|vue|svelte|html|weapp>`
  ...
  - 注入样式引用（如全局或局部引入 `@timui/shared/button.css`）。
...
- [ ] 按照 Schema 重构 React 组件 (`packages/react`)。
...
- 包产物：`@timui/tokens`（CSS/JSON/TS/WXSS）、`@timui/shared`、`@timui/react|vue|svelte|html|weapp`、`@timui/cli`。
